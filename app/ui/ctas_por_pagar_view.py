# control_negocio/app/ui/ctas_por_pagar_view.py
...
class CtasPorPagarView(Frame):
    def __init__(self, master=None):
        ...
        # StringVar debe recibir 'self' como master
    ...
    def crear_widgets(self):
        Label(self, text="📋 Cuentas por Pagar", font=("Arial", 16, "bold"), bg="white")\
            .pack(pady=10)
        ...
        Label(form, text="Proveedor:", bg="white").grid(row=1, column=0, sticky="e", padx=5, pady=2)
        self.ent_proveedor = Entry(form, width=20)
        self.ent_proveedor.grid(row=1, column=1, padx=5, pady=2)
        ...
        # Tabla de resultados
        cols = ("ID", "Número", "Proveedor", "Monto", "Estado", "Fecha")
    ...
    def _on_select(self, _):
        ...
        if not sel:
            self.cta_sel_id = None
            return
        id_, num, prov, mon, est, fech = self.tree.item(sel[0])["values"]
        self.cta_sel_id = id_
        self.ent_numero.delete(0, tk.END);     self.ent_numero.insert(0, num)
        self.ent_proveedor.delete(0, tk.END);  self.ent_proveedor.insert(0, prov)
    ...
    def cargar_ctas(self):
        for r in self.tree.get_children():
            self.tree.delete(r)
        facturas = [f for f in Finanzas.listar_facturas() if f[6] == "proveedor"]
